...
import { useHeightAnimation } from "@ah/hooks/devices/useHeightAnimation";
...
export const NavItem: React.FC<items> = ({
  ...
  const { id, label, link = "#", iconName, children } = items;
  const { openItems, toggleNavItem } = useSidebar();
  const isOpen = openItems[id];
  const hasChildren = children?.length;

  const heightRef = useHeightAnimation(isOpen);

  return (
    <li className={`mb-1 border-b border-[var(--color-border)] ${className}`}>
      {hasChildren ? (
        <Button
          variant={"ghost"}
          color={"primary"}
